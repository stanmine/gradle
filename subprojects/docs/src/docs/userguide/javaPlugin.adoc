...
* `gradle test --tests "MyTestSuite"`
* `gradle test --tests "com.example.ParameterizedTest"`
* `gradle test --tests "ParameterizedTest"`
...
[NOTE]
====
Since `--tests` supports method name matching, Gradle has to load all possible classes, which may lead to very bad performance, especially with api:org.gradle.api.tasks.testing.Test#getForkEvery()[`forkEvery`].
To help Gradle exclude as many classes as possible, please don't use a pattern starting with a wildcard (*).
====
